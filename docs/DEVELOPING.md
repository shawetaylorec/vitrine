...
| `_p2.js` | hit testing, dragging, keyboard, preview mode, the register, the inspector |
...
Console lines are tagged `[PASS]` / `[FAIL]`. There are 363 assertions covering the
...
| `#tight` / `#darktight` | both rails collapsed — the register as a strip |
| `#keys` | the keyboard card |
...
`_shell.html` defines two sets of custom properties and they must not be mixed.

**The drawing's inks** — `--sheet`, `--ink`, `--accent`, `--plan`, `--case-fill`,
`--struct` and the rest — are read by `readPalette()` into `C` and painted onto the
canvas. `--ink` is *lettering on the sheet*, not interface text. Change one of these and
you change the drawing, and the pixel assertions with it.

**The instrument** — `--chrome`, `--chrome-2/3/4`, `--seam`, `--seam-2`,
`--chrome-ink*`, `--filament` — is the interface, and is never drawn. All chrome CSS
uses these. That separation is why the interface could be rebuilt without touching a
single drawn pixel: the drawing kept its own colours throughout.

The direction: a vitrine's mount disappears around what it holds, so the chrome is dark,
quiet metal and the sheet is the only lit thing in the room. **Selection reads as light
rather than as colour** — a lifted plane and a bright hairline — which keeps the one warm
hue, `--filament`, for the few things that are genuinely live: the primary action, an
armed tool, the lock. The type follows the same split: IBM Plex Sans Condensed and Plex
Mono for the instrument, since Plex was drawn for technical documentation and the numbers
were already Plex Mono; Bodoni Moda, used sparingly, for the wordmark, modal titles and
the case's own name, which is the typography of an exhibition rather than of a tool. The
canvas keeps **Barlow Semi Condensed** (`UIFONT`) — panel wording is measured and wrapped
in it, so changing it would reflow the owner's scholarly text for no gain.

## The case is a thing you inspect

The right rail shows what is selected. Select nothing and it shows **the case**: its
dimensions, the rail height, the back wall, the measuring lines. That is not an empty
state dressed up — the case has dimensions like any other object, and its controls used
to sit in the left rail, one panel away from the readout that described them.

The controls are **real nodes**, not markup rebuilt on each render. They live in
`#stash > #caseTools`, `_p4.js` binds every listener to them once at boot, and
`renderCaseInspector()` moves that subtree into `#caseToolsSlot` and `stashCaseTools()`
hands it back. Rebuilding them from a template string instead would kill every listener
the first time anything was selected — which is what the assertions in *7m2* are there to
catch. For the same reason the panel is only rebuilt when it is not already standing: a
`commit()` while you are typing in `#caseW` must not move the node out from under the
caret.

## The register has its own order

`ord` is a per-group index that **only the rail reads**. `railOrder()` sorts a copy;
`railGroup()` says which of the three groups an item belongs to; `reorderRail()` and
`nudgeRail()` renumber one group 0…n−1. An item that has never been moved has no `ord`
and sorts after everything that has, on `ORD_END + index`, so new arrivals land at the
foot of their group and a case that has never been tidied lists exactly as it always did.

**It must never be the order of `S.items`, and that is not a stylistic preference.**
Three things read that array positionally:

- `drawFront()` sorts by `depthKey` with a *stable* sort, so ties are broken by array
  position;
- `drawPlan()` iterates the array outright;
- `hitTest()` resolves a tie by taking the later item — "because that is the one drawn
  last and so the one you can actually see".

Move a row in `S.items` to tidy a list and you would silently change which of two things
at the same depth is drawn on top, and which one a click picks up. The owner asked for a
filing order that changes nothing about the design; `ord` is what makes that true.

It rides in the case file rather than in `localStorage`, because it belongs to the case
and should travel with it. Nothing had to be added to the save path: `undoState()`
spreads each item and `upgrade()` merges the whole object, so a new field survives a round
trip on its own. `duplicate()` deletes it from the copy, so a copy joins the end of its
group rather than sharing the original's place.

## The rails

`body.noL` / `body.noR` collapse a rail by changing `--wL` / `--wR`, which the `.app`
grid transitions. `toggleRail(side)` is global so the keyboard handler in `_p2.js` can
reach it from `[` and `]`.

**It is not saved with the case.** How much of the drawing you want to see is a fact
about the person and the screen, not about the vitrine — and the round-trip test compares
the saved file byte for byte, so anything added to `S` has to be worth that. It rides in
`localStorage` under `vitrine-rails`, with the theme and the folded groups.

Collapsed, the left rail is **not empty**: the same register rows stay in the document
and CSS hides everything but the swatch, leaving a strip of the objects themselves —
clickable, and still showing the selection. One list, two densities, no second code path.
